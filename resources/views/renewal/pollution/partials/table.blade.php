@if ($renewal_lists->count() > 0)
    @foreach ($renewal_lists as $key => $vehicle)
        ...
        <tr>
            ...
            <td>{{ $pollution->invoice_no ?? '-' }}</td>
            <td>{{ $vehicle->owner->first_name }} {{ $vehicle->owner->last_name }}</td>
            <td>{{ $vehicle->vehicleType->name }}</td>
            <td>{{ $vehicle->registration_no }}</td>
            <td>{{ $pollution->last_expiry_date ?? '-' }}</td>
            <td>{{ $pollution->expiry_date ?? '-' }}</td>
                ...
                <ul class="list-inline hstack gap-2 mb-0">
                    ...
                    @if($pollution)
                        <li class="list-inline-item" title="Edit">
                            <a href="{{ route('admin.renewal.pollution.edit', $pollution->id) }}">
                        ...
                        {{-- <li class="list-inline-item" title="View">
                            <a href="{{ route('admin.renewal.pollution.show', $pollution->id) }}">
