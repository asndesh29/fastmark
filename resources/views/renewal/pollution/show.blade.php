    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <h4 class="card-title mb-0">Renewal History for {{ $pollution->vehicle->registration_no }}</h4>
                <a href="{{ route('admin.renewal.pollution.index') }}" class="btn btn-primary btn-md">← Back</a>
            </div>
            ...
            <div class="card-body">
                <div class="table-responsive table-card mt-3 mb-1">
                    <table class="table align-middle">
                        ...
                        <tbody>
                            @if($pollution->vehicle->renewals->count())
                                @foreach($pollution->vehicle->renewals as $renewal)
                                    <tr>
                                        <td>{{ $renewal->renewalType->name ?? '—' }}</td>
                                        <td>{{ $pollution->issue_date }}</td>
