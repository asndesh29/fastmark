...
{
    // Data for Nepali date, limited to 2000-2089 BS
    private $_bs = array(
        ...
        90 => array(2090, 31, 31, 32, 31, 31, 31, 30, 29, 30, 29, 30, 30),
        91 => array(2091, 31, 32, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30)
    );
    ...
    {
        $index = $year - 2000;
        if ($index < 0 || $index >= count($this->_bs)) {
            throw new \Exception("Year $year is out of supported range (2000-2090 BS)");
        }
        if ($month < 1 || $month > 12) {
            throw new \Exception("Month must be between 1 and 12");
        }
        return $this->_bs[$index][$month];
    }
}
